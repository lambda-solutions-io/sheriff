...
make stray barrels loud: `'warn'` prints a warning in `sheriff verify`,
`'forbid'` turns every barrel module into a violation. The new `barrel-policy`
ESLint rule reports directly on the barrel file, so the finding shows up in
the editor and in CI lint runs.
